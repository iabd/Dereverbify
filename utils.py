...
def get_checkpoint_dir():
  ...
  checkpoint_dir="/content/gdrive/MyDrive/Research/Dereverbify/checkpoints/{}".format(today)

  if not os.path.exists(checkpoint_dir):
    os.makedirs(checkpoint_dir)
  return Path(checkpoint_dir)

def save_model_weights(net, filename, verbose=1, cp_folder=""):
    if verbose:
        print(f"\n -> Saving weights to {os.path.join(cp_folder, filename)}\n")
    torch.save(net.state_dict(), os.path.join(cp_folder, filename))
